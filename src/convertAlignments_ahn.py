...
pkl_filepath = sys.argv[1]  # 'wordTranscriptions.pkl'
in_tg_path = sys.argv[2]
out_tg_dir = sys.argv[3]

if not os.path.exists(out_tg_dir):
    os.makedirs(out_tg_dir)

# Read data from file:
print(f"Will read from the file: {pkl_filepath}.")
...
for textGridFilePath in glob.iglob(in_tg_path + '**/*.TextGrid', recursive=True):
...
# Verify that the TextGrid has only two tiers: words and phones
    ...
    out_tg_file = os.path.join(out_tg_dir, os.path.basename(textGridFilePath))
    tgt.write_to_file(tg,out_tg_file,format='long')
